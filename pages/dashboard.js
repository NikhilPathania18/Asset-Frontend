...
const Dashboard = () => {
  ...
  const router = useRouter();
    useEffect(() => {
      let data = localStorage.getItem("superAdmin");
      if (!data) return;
  ...
  <aside id="logo-sidebar" className="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
    <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
      <p className=" font-semibold text-blue-700 ml-3 ">{user}</p>
      <ul className="space-y-2 font-medium">
        ...
        <li>
          <a href="#" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
            ...
            {
              signedIn && (<span className="flex-1 ml-3 whitespace-nowrap" onClick={signOut}>Sign Out</span>) 
               
            }
            {
              !signedIn &&(<span className="flex-1 ml-3 whitespace-nowrap" onClick={signIn}>Sign In</span>)
            }
